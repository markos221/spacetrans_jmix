...
> {
  dataCollection = collection<Waybill>(Waybill.NAME, {
  ...
  showDeletionDialog = (e: SerializedEntity<Waybill>) => {
    Modal.confirm({
  ...
  render() {
    ...
    const buttons = [
      <EntityPermAccessControl
        ...
        key="create"
      >
        <Link to={WaybillManagement.PATH + "/" + WaybillManagement.NEW_SUBPATH}>
          <Button
            htmlType="button"
      ...
      <EntityPermAccessControl
        ...
        key="update"
      >
        <Link to={WaybillManagement.PATH + "/" + this.selectedRowKey}>
          <Button
            htmlType="button"
            style={{ margin: "0 12px 12px 0" }}
  ...
  getRecordById(id: string): SerializedEntity<Waybill> {
    const record:
      | SerializedEntity<Waybill>
...
const WaybillList = injectIntl(injectMainStore(observer(WaybillListComponent)));
