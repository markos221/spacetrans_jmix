...
> {
  dataCollection = collection<Carrier>(Carrier.NAME, {
  ...
  showDeletionDialog = (e: SerializedEntity<Carrier>) => {
    Modal.confirm({
  ...
  render() {
    ...
    const buttons = [
      <EntityPermAccessControl
        ...
        key="create"
      >
        <Link to={CarrierManagement.PATH + "/" + CarrierManagement.NEW_SUBPATH}>
          <Button
            htmlType="button"
      ...
      <EntityPermAccessControl
        ...
        key="update"
      >
        <Link to={CarrierManagement.PATH + "/" + this.selectedRowKey}>
          <Button
            htmlType="button"
            style={{ margin: "0 12px 12px 0" }}
  ...
  getRecordById(id: string): SerializedEntity<Carrier> {
    const record:
      | SerializedEntity<Carrier>
...
const CarrierList = injectIntl(injectMainStore(observer(CarrierListComponent)));
