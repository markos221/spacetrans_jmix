...
class SpaceportListComponent extends React.Component<
  MainStoreInjected & WrappedComponentProps
> {
  dataCollection = collection<Spaceport>(Spaceport.NAME, {
    view: "spaceport-fetch-plan"
  });
  selectedRowKey: string | null = null;

  fields = ["name", "isDefault", "planet", "moon"];

  showDeletionDialog = (e: SerializedEntity<Spaceport>) => {
    Modal.confirm({
  ...
  render() {
    ...
    const buttons = [
      ...
      >
        <Link
          to={SpaceportManagement.PATH + "/" + SpaceportManagement.NEW_SUBPATH}
        >
          <Button
            htmlType="button"
      ...
      <EntityPermAccessControl
        ...
        key="update"
      >
        <Link to={SpaceportManagement.PATH + "/" + this.selectedRowKey}>
          <Button
            htmlType="button"
            style={{ margin: "0 12px 12px 0" }}
  ...
  getRecordById(id: string): SerializedEntity<Spaceport> {
    const record:
      | SerializedEntity<Spaceport>
...
const SpaceportList = injectIntl(
  injectMainStore(observer(SpaceportListComponent))
);
